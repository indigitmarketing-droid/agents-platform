...
export function MetricsBar({ events }: MetricsBarProps) {
  ...
  const metrics = [
    ...
    { label: "Calls Made", value: events.filter((e) => e.type === "setting.call_initiated" || e.type === "setting.sales_call_initiated").length },
    { label: "Sites Built", value: events.filter((e) => e.type === "builder.website_ready").length },
    { label: "Sales Closed", value: events.filter((e) => e.type === "customer.onboarded").length },
  ];
