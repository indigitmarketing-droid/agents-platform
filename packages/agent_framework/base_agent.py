...
class BaseAgent(ABC):
    ...
    async def process_event(self, event: dict) -> None:
        ...
        try:
            self._emitter.set_status("working")
            (
            ...
            if should_retry(retry_count, self.MAX_RETRIES):
                ...
                self._mark_dead_letter(event_id, str(e))
            self._emitter.set_status("idle")
        except (FatalError, Exception) as e:
            logger.error(f"[{self.agent_id}] Fatal error on {event_id}: {e}")
            self._mark_dead_letter(event_id, str(e))
            self._emitter.set_status("error")

    def _mark_dead_letter(self, event_id: str, error: str) -> None:
        (
            self._client.table("events")
    ...
    async def stop(self) -> None:
        self._running = False
        self._emitter.set_status("offline")
