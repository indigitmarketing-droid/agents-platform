...
def mock_supabase():
    ...
    table = MagicMock()
    insert = MagicMock()
    insert.execute = AsyncMock(return_value=MagicMock(data=[{"id": "test-uuid"}]))
...
@pytest.mark.asyncio
async def test_emit_event(mock_supabase):
...
@pytest.mark.asyncio
async def test_emit_broadcast_event(mock_supabase):
    emitter = EventEmitter(client=mock_supabase, agent_id="scraping")
    await emitter.emit(
...
@pytest.mark.asyncio
async def test_heartbeat(mock_supabase):
    emitter = EventEmitter(client=mock_supabase, agent_id="scraping")
    await emitter.send_heartbeat()
